...
_INSTALL_MESSAGE = "Install them from docs/INSTALLATION.md and restart the standalone FlashVSR environment."
...
def _dependency_import_message(display_name: str, module_name: str, exc: BaseException) -> str:
    return f"{_REQUIREMENTS_MESSAGE} {display_name} is installed, but importing {module_name} failed. Check the console for the import error, then reinstall from docs/INSTALLATION.md and restart the standalone FlashVSR environment. Import failed: {type(exc).__name__}: {exc}"


def _kernel_load_message(sparge_error: str | None) -> str:
    return f"{_REQUIREMENTS_MESSAGE} SpargeAttn is installed, but its kernels could not be loaded. Reinstall SpargeAttn from docs/INSTALLATION.md and restart the standalone FlashVSR environment. SpargeAttn import failed: {sparge_error or 'not installed'}"


def _arch_kernel_load_message(arch: str, module_name: str, exc: BaseException | None) -> str:
    if exc is not None:
        return f"{_REQUIREMENTS_MESSAGE} SpargeAttn is installed, but importing its {arch} kernel failed. Check the console for the import error, then reinstall SpargeAttn from docs/INSTALLATION.md and restart the standalone FlashVSR environment. Import failed: {type(exc).__name__}: {exc}"
    return f"{_REQUIREMENTS_MESSAGE} SpargeAttn is installed, but its {arch} kernel is unavailable. Reinstall SpargeAttn from docs/INSTALLATION.md and restart the standalone FlashVSR environment. Missing kernel module: {module_name}"
...
def _import_sparge_core():
    try:
        return importlib.import_module("flashvsr.spas_sage_attn_core"), None
    except Exception as exc:
        _print_import_error("flashvsr.spas_sage_attn_core", exc)
...
def _load_triton_sparse_backend() -> tuple[Callable | None, str | None, str | None]:
    try:
        ...
        _print_import_error("flashvsr.sparse_sage.core", exc)
        return None, None, _dependency_import_message(_BUNDLED_SPARSE_BACKEND_NAME, "flashvsr.sparse_sage.core", exc)
...
def _backend_requirement_status(backend: str) -> tuple[Callable | None, str | None, str | None]:
    ...
    if not callable(fn):
        return None, None, _kernel_load_message("FlashVSR SpargeAttn block sparse CUDA function not found")
    return fn, "FlashVSR SpargeAttn block sparse CUDA", None
...
def _load_backend() -> tuple[Callable, str]:
    ...
    use_qkv_list = sparge_fn.__module__ == "flashvsr.spas_sage_attn_core"
