...
def _save_still_image_debug_video(frames: torch.Tensor) -> None:
    # WanGP's optional debug-video writer depended on shared.utils.audio_video.
    # It is deliberately omitted from the standalone engine. Normal FlashVSR
    # processing is unaffected because this feature is disabled by default.
    if not FLASHVSR_SAVE_STILL_IMAGE_DEBUG_VIDEO:
        return
    print(
        "[FlashVSR] Still-image debug-video export is not available in the "
        "standalone engine. Set FLASHVSR_SAVE_STILL_IMAGE_DEBUG_VIDEO=False."
    )
