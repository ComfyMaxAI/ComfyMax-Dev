...
with editor:
    ...
    options = [""] + mapped_workflows(ROOT)
    previous_workflow = state["workflow"]
    if previous_workflow not in options:
        options.append(previous_workflow)
        st.warning("This scene's saved workflow is unavailable. Choose an installed mapped workflow.")
    state["workflow"] = st.selectbox("Scene workflow", options, index=options.index(previous_workflow),
        format_func=lambda value: value or "Choose workflow", key=prefix + "workflow")
    if state["workflow"] != previous_workflow:
        state["inputs"] = {}
        state["assets"] = {}
    ...
    if state["workflow"]:
        try:
            workflow, mapping, digest = load_mapped_workflow(ROOT, state["workflow"])
        ...
        problems.append("Choose a mapped workflow.")
    ...
    generation_allowed = bool(model and mapping and not problems and replace_prompt and not busy)
        ...
        try:
            ...
            def _generate_scene_prompt():
                return lm.generate_prompt(
                    compose_prompt(project, scene, mapping),
                    model,
                    prompt_system(
                        mapping,
                        values.get("duration"),
    ...
    if approve.button("Approve scene prompt", disabled=not state["prompt"].strip() or bool(problems) or busy):
        project[KEY]["selected_scene_id"] = selected
    ...
    if st.button(
        render_label,
        type="primary",
        disabled=not approved or bool(problems) or busy or bool(st.session_state.get("mvd_model_instance")),
    ):
        state["render"] = {
            "state": "submitting",
